...
model = KM.Model()
base_model_part = model.CreateModelPart("BasePart")
base_model_part.AddNodalSolutionStepVariable(KM.VELOCITY) ##needed to compute the normal
base_model_part.AddNodalSolutionStepVariable(KM.TEMPERATURE) ##needed to compute the normal


#reading the base model part
model_part_io_fluid = KM.ModelPartIO("base")
...
for node in base_model_part.Nodes:
    ...
    if( r < 0.45):
        node.SetSolutionStepValue(KM.VELOCITY_X, 0, -ylocal)
        node.SetSolutionStepValue(KM.VELOCITY_Y, 0, xlocal)
    
        d = math.sqrt( (xlocal - xc)**2 + (ylocal-yc)**2 ) - 0.1
        
        node.SetSolutionStepValue(KM.TEMPERATURE, 0, d)
        #if(d <= 0.0):
...
locator = KM.BinBasedFastPointLocator2D(base_model_part)
...
while( t < final_time):
    ...
    print("time = ",t)
    bfecc_utility.BFECCconvect(base_model_part, KM.TEMPERATURE, KM.VELOCITY, substepping)
